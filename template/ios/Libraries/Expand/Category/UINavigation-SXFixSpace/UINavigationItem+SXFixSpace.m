...
-(void)sx_setLeftBarButtonItems:(NSArray<UIBarButtonItem *> *)leftBarButtonItems {
    if (sx_deviceVersion >= 11) {
        ...
        if (leftBarButtonItems.count) {
            NSMutableArray *items = [NSMutableArray arrayWithObject:[self fixedSpaceWithWidth:[UINavigationConfig shared].sx_defaultFixSpaceLeft-20]];//可修正iOS11之前的偏移
...
-(void)sx_setRightBarButtonItems:(NSArray<UIBarButtonItem *> *)rightBarButtonItems{
    if (sx_deviceVersion >= 11) {
        ...
        if (rightBarButtonItems.count) {
            NSMutableArray *items = [NSMutableArray arrayWithObject:[self fixedSpaceWithWidth:[UINavigationConfig shared].sx_defaultFixSpaceRight-20]];//可修正iOS11之前的偏移
